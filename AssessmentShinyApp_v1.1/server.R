...
shinyServer(function(input,output,session){
  ...
  initialResults <- eventReactive(input$runButton,{withProgress(message='Processing Sites',value=0,{
    ...
    WQS <- readOGR(parseDirPath(volumes, input$directory),'wqs_riverine_id305b_2013_albers')
    NWBD <- readOGR(parseDirPath(volumes, input$directory),'vaNWBD_v4_albers')
  ...
  observe({if(input$runButton2==T){withProgress(message='Processing Sites',value=0,{
    ...
    WQS_p <- readOGR(parseDirPath(volumes, input$directory),'wqs_riverine_id305b_2013_Planarized84')
    ...
    output$issueMap <- renderLeaflet({
      leaflet() %>% addProviderTiles('Thunderforest.Outdoors') %>%
        ...
        addMarkers(data=problemsites_tbl(),popup=paste(sep="<br/>",strong(problemsites_tbl()$StationID)
                                                       ,problemsites_tbl()$Comment)) %>%
        addPolylines(data=WQS_p_selection,color=~pal(ID305B), weight=5,group=WQS_p_selection@data$ID305B
                     ,popup=paste(sep = "<br/>",strong('Stream Name: '),WQS_p_selection@data$WATER_NAME
                                  ,strong('ID305B:'),WQS_p_selection@data$ID305B
                                  ,strong('WQS Section: '),WQS_p_selection@data$WQS_SECTIO
                                  ,strong('WQS Class: '),WQS_p_selection@data$WQS_CLASS
                                  ,strong('WQS Trout: '),WQS_p_selection@data$WQS_TROUT
                                  ,strong('WQS Special Standards: '),WQS_p_selection@data$WQS_SPECIA))})
  })
  }})
  ...
  userValues$df <- data.frame(Depth=numeric(0),ID305B_2=numeric(0),ID305B_3=numeric(0),STATION_TYPE_1=numeric(0)
                              ...
                              ,WATER_MET_VIO=numeric(0),WATER_MET_STAT=numeric(0),WATER_TOX_VIO=numeric(0),WATER_TOX_STAT=numeric(0)
                              ,SED_MET_VIO=numeric(0),SED_MET_STAT=numeric(0),SED_TOX_VIO=numeric(0),SED_TOX_STAT=numeric(0)
